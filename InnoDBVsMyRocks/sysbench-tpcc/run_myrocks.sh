...
./tpcc.lua --mysql-host=10.0.4.112 --mysql-user=sysbench --mysql-password=sysbench --mysql-ssl=off --mysql-db=$db --threads=4 --scale=200 --use_fk=0 --mysql_storage_engine=rocksdb --mysql_table_options="COLLATE latin1_bin" --trx_level=RC --report-interval=10 --time=$duree run > $1/sb.out
mysql -h 10.0.4.112 -u sysbench -psysbench -e 'show global status' > $1/mysql_status_end.out
ssh 10.0.4.112 'df -h' > $1/fs_size_end.out 
